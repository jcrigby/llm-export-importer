...
function generateTestSamples(): { content: string; expectedScore: number; type: string }[] {
  return [
    ...
    {
      content:
        "Sarah's hands trembled as she reached for the letter. The envelope bore her father's handwriting, impossible since he'd been dead for three years. She tore it open, heart pounding, and read the first line: 'My dearest daughter, if you're reading this, then the curse has finally found our family.'",
      expectedScore: 9,
      type: 'fiction',
    },
    {
    ...
    {
      content:
        "Can you help me brainstorm some character names for my fantasy novel? I need something that sounds elvish but not too cliché. The main character is a female archer who's also skilled in magic.",
      expectedScore: 6,
    ...
    {
      content:
        "What's 2 + 2? I need to solve this math problem quickly. Also, can you explain the quadratic formula?",
      expectedScore: 0,
      type: 'math',
    },
    {
      content:
        "Hey how's the weather today? I'm thinking about going to the grocery store later. Do you have any recommendations for what I should buy?",
      expectedScore: 0,
      type: 'casual-chat',
    },
  ];
...
async function validateModel(
  modelId: string,
  testSamples: ReturnType<typeof generateTestSamples>
...
export async function recommendOptimalModel(estimatedTokens: number): Promise<ModelRecommendation> {
  ...
  for (const model of models) {
    ...
    console.log(
      `     Cost: $${estimatedCost.toFixed(3)} (${model.pricing.prompt.toFixed(6)}/token)`
    );
    ...
    if (validation.suitable) {
      ...
      console.log(
        `   • ${(validation.accuracy * 100).toFixed(1)}% accuracy (exceeds 75% threshold)`
      );
